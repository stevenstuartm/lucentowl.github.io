...
description: "How singly and doubly linked lists store elements as nodes joined by pointers, why insertion is O(1) only once you hold the node, what .NET's LinkedList<T> provides, where linked lists beat arrays, and the classic pointer techniques: reversal, cycle detection, and finding the middle."
...
An array keeps its elements side by side, so inserting or removing anywhere but the end shifts every element after that point. A linked list gives up contiguity to avoid the shifting. Each element lives in its own node, and each node holds a pointer to the next one. The order of the list lives in those pointers, not in memory addresses, so inserting a node means changing a couple of pointers, however long the list is.
...
The `Previous` pointer costs one more reference per node, so a doubly linked node carries two pointers where a singly linked node carries one. In exchange, a node can unlink itself in O(1), because it can reach both neighbors directly:
...
The doubly linked list also allows walking backward from the tail. Most practical uses of linked lists, including .NET's own, are doubly linked, because O(1) removal of a held node is usually the reason for choosing a linked list at all.

---

## LinkedList&lt;T&gt; in .NET

`System.Collections.Generic.LinkedList<T>` is a doubly linked list. Internally it is circular, meaning the head's `Previous` pointer points to the tail, although its public `Next` and `Previous` properties return `null` at the ends. Its API is built around `LinkedListNode<T>` references:

| Member | Cost | Notes |
| --- | --- | --- |
| `AddFirst`, `AddLast` | O(1) | Return the new `LinkedListNode<T>` |
...
Each `LinkedListNode<T>` is a separate object on the heap that holds references to its list, its next node, and its previous node, plus the value itself. With the object header that every .NET object carries, a node storing one `int` takes several times the memory of that `int` in an array.

---

## Where Linked Lists Win and Lose

Linked lists lose most head-to-head comparisons with dynamic arrays in practice, even where their Big O is equal or better. An array scan reads memory sequentially, which the CPU cache serves efficiently. A linked list scan follows pointers to nodes scattered across the heap, and each hop can miss the cache. Every node is also a separate allocation for the garbage collector to track.
...
Linked lists also fit when many insertions and removals happen at positions the code is already visiting, such as filtering or splicing a list during a single pass, and when node references must stay valid while other elements are added and removed. An element's index in an array changes every time something before it is inserted, but a node reference stays the same.

---

## Classic Pointer Techniques

These three problems show up constantly because each one tests careful pointer handling in a few lines. They work on bare nodes rather than a list class:
