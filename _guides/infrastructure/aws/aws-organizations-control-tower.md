...
An organization has one **management account** and any number of **member accounts**, arranged in a tree:

| Element | What it is |
|---|---|
| **Management account** | The account that created the organization. It pays the bill for every member, can create and remove accounts, and manages organization policies. Organization authorization policies (SCPs and RCPs) never restrict it |
...
Set up **centralized root access** for IAM early. With it enabled, the management account or its IAM delegated administrator can remove the root user credentials from every member account, and new accounts are created with none. The rare tasks that need root, such as deleting a bucket policy that locks everyone out, run centrally as privileged actions instead.
...
Add any other global services the organization uses to the `NotAction` list. Control Tower offers the same restriction as a built-in Region deny control.
...
A landing zone is the organization plus the shared accounts and services Control Tower manages. Since **landing zone version 4.0**, each service integration is optional, so a landing zone contains whichever of these the organization turns on:

- **A log archive account**, which collects CloudTrail and Config logs from every account in an S3 bucket that workload teams can't change.
- **An audit (security) account**, which holds the security roles that give the security team cross-account read access and receives notifications.
- **IAM Identity Center**, configured for access across the accounts. This depends on the security roles integration.
- **AWS Backup**, with a central backup vault.
- **Account Factory**, which creates new accounts with the baseline already applied.
- **A dashboard** showing accounts, enabled controls, and non-compliant resources.

Control Tower no longer creates or requires a particular OU structure. Whichever OU holds the log archive and audit accounts becomes the Security OU, and the organization defines the rest. Older guides describe a mandatory Security OU and Sandbox OU created at setup, which was true of earlier landing zone versions.
...
|---|---|---|---|
| **Preventive** | Before an API call succeeds | SCPs and RCPs | Disallow changes to the log archive bucket's policy |
...
### Control Tower or Organizations Alone

| Choose Control Tower when | Use Organizations alone when |
|---|---|
| Building a multi-account environment and wanting AWS's baseline without assembling it | The organization already has a mature, custom baseline built on Organizations and IaC |
| Teams need self-service account vending with governance built in | Control Tower's managed resources would conflict with existing logging or security tooling |
| Auditors expect a dashboard of controls and compliance | The team wants to own every SCP and Config rule directly |

With optional integrations, the gap between the two is narrower than it was. Many organizations use Control Tower for vending and a small set of controls, and manage the rest through their own IaC.
...
Structure OUs by the policies they need, not by the org chart. An OU earns its place when its accounts share controls that differ from their neighbors'. Splitting production from non-production under Workloads is the most common first division, because the two need different SCPs.
...
aws ram create-resource-share \
  --name "Shared-TGW" \
  --resource-arns "arn:aws:ec2:us-east-1:111111111111:transit-gateway/tgw-0example" \
...
SCPs and RCPs don't apply to the management account, so a workload there runs without guardrails, and a breach there reaches billing and every organization policy. Move workloads out and keep access to the management account small and heavily monitored.

### Untested Policies at the Root

A deny attached to the root applies everywhere at once, including to production. Roll policies out through a Policy Staging OU, and keep an exception process ready for the day an SCP blocks something a production incident needs. The same goes for identity: if IAM Identity Center or the external identity provider is the only way into any account, keep a tested emergency path that doesn't depend on it.

### OUs That Mirror the Org Chart

An OU structure copied from reporting lines tends to need rework, because reorganizations don't change what policies a workload needs. Group accounts by the controls they share, and nest OUs where a subset of accounts needs stricter rules.
...
5. **Control Tower builds and watches a landing zone.** Its controls are SCPs and RCPs (preventive), Config rules (detective), and CloudFormation hooks (proactive), applied per OU. Landing zone 4.0 made its integrations and OU structure optional.
