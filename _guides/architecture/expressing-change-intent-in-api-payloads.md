...
This lever belongs in a collection's design as a default whenever its elements have their own identity, not just as a fix once the large-collection pressure appears, since a dedicated endpoint costs close to nothing and removes the lost-update risk from that slice of the resource entirely. The same reasoning holds in RPC: a dedicated `AddLabel`/`RemoveLabel` method instead of a generic `UpdateIssue` carrying a repeated field is the identical lever, just rarely named as such outside REST. It fits less naturally for anonymous, positional collections, an ordered list of steps with no natural per-element key, where there's nothing to hang a sub-resource endpoint on.
