...
- **Per guide:** run the content items, apply the fixes, run the presentation pass, then the independent review from [Writing a New Guide](study-guide-guide.md#writing-a-new-guide) (steps 4 and 5), for at most two rounds. That review uses the `guide-reviewer` agent, on the session's model for round 1 and Sonnet for round 2. Each round's fixes go through item 1 and `/refine-prose` again before the next round or the status change, because the presentation pass that ran before the review doesn't cover text written after it. Then set status to Complete and move to the next row. The first review is mandatory for every guide in a depth pass, whether it was lightly corrected or rewritten, and a plan's cadence can't waive it. The second round is optional (see step 5). Don't dispatch a third. Don't stop for approval between rows. No Jekyll build — content-only edits don't break the build.
...
- **`Complete` means the content items ran, the presentation pass finished with `/refine-prose` clean, and the independent review ran (at most two rounds) with every finding applied or rejected against a source.** Not "the diff looks substantial." Item 1 leaves no trace in a diff, so this status is the only record that it happened; setting it early destroys the information permanently.
