...
- **Per guide:** run the content items, apply the fixes, run the presentation pass, then the independent review from [Writing a New Guide](study-guide-guide.md#writing-a-new-guide) (steps 4 and 5), for at most two rounds. Then set status to Complete and move to the next row. The first review is mandatory for every guide in a depth pass, whether it was lightly corrected or rewritten, and a plan's cadence can't waive it. The second round is optional (see step 5). Don't dispatch a third. Don't stop for approval between rows. No Jekyll build — content-only edits don't break the build.
