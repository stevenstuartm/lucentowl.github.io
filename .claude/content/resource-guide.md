...
- **description**: One sentence, 250 characters at most, that states the resource's **scope**, not its inventory. Write it to stand alone in a listing card. Say what territory the resource covers and what kind of help it gives, at a level that stays true as entries are added. "How a desktop app's markup, layout, controls, data, and threads fit together" survives a new figure. A list of every figure, or a sentence opening "starting with…", doesn't. **Adding an entry, a figure, or a table row never requires editing the description.** If a new entry falls outside the stated scope, either the entry belongs in a different resource, or the scope sentence was too narrow and gets rewritten once at a broader level, not extended by another clause.
- **tags**: 3 to 6, naming the resource's domain the way a guide's tags do. Tags aren't a per-entry index either, so don't add a tag for each new figure or section.
