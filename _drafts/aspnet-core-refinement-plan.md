...
- **Shutdown.** `HostOptions.ShutdownTimeout` defaults to 30 seconds (5 seconds before .NET 6). Sequence: ApplicationStopping, then hosted services stop in reverse registration order. The server (`GenericWebHostService`) is registered after the app's own services in `WebApplicationBuilder.Build`, so it starts last and stops first: it drains before any `BackgroundService` token is cancelled, and the drain counts against the timeout (unless `ServicesStopConcurrently`). Then ApplicationStopped. (Corrected at row 17; row 1 fixed.)
...
- **Background services (row 17 owns).** Since .NET 6 an unhandled `ExecuteAsync` exception stops the host (`BackgroundServiceExceptionBehavior.StopHost`). The host excuses an `OperationCanceledException` only while `ApplicationStopping` is cancelled, so catch filters use `when (!stoppingToken.IsCancellationRequested)`. Since .NET 10 `ExecuteAsync` runs on the thread pool. Quartz.NET 4.0 (Sept 2026) folded `Quartz.Extensions.Hosting` into `Quartz`; Quartz has no automatic retry; `Quartz.Dashboard` is a work in progress. Hangfire: 10 retries, at-least-once, dashboard local-only by default. The `asp-hosted-service-lifetime` figure exists for reuse.
...
| Target row | Lead |
|---|---|
| 18 health-checks | Resume here (rows 1-17 Complete). Integrate moved-in HTTP logging and background-service health sections. Telemetry-pillars and correlation-middleware sections cut to clauses (Observability and C# logging own). `MapPrometheusScrapingEndpoint` needs the prerelease exporter package; check status. |
...
| 17 | Testing & Operations | aspnet-background-services.md | Complete |
