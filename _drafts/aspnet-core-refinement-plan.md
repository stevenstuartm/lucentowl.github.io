...
| Target row | Lead |
|---|---|
| 15 api-testing | Resume here (rows 1-14 Complete). Remove the architecture-testing section (not ASP.NET-specific). `services.RemoveAll<DbContext>()` does not remove `DbContextOptions<T>`; the sample is likely wrong. Respawn sample uses an instance field in a static initializer. |
