...
| Target row | Lead |
|---|---|
| 4 minimal-apis | Remove the validation section (row 6 owns) and the manual SSE section (row 10 owns; superseded by `TypedResults.ServerSentEvents`). Merge the moved-in "Minimal API vs Controller Performance" section into "Philosophy and Choosing" and "Performance Characteristics". `WithOpenApi()` is deprecated in .NET 10, and "minimal APIs are the only supported model under AOT" needs checking. OpenAPI metadata section shrinks to a clause (row 8 owns). Add the `ConfigureHttpJsonOptions` vs `AddJsonOptions` gotcha. |
| 5 controller-apis | Two content-negotiation/formatter treatments now sit side by side; merge into one. Drop the route-constraint table (row 3 owns). "Problem Details … RFC 7807" is superseded by RFC 9457. |
| 6 validation | JSON configuration section drops to a clause (C# JSON guide owns). Title is now "Validation"; description and tags must follow. |
| 7 large-payloads | New seed: write the guide around the moved sections (three overlapping upload treatments to merge), then run the full checklist with no discount on item 1. Kestrel's default body limit is 30,000,000 bytes (~28.6 MiB), which rows 6 and 13 stated two ways; state it once. `[DisableFormValueModelBinding]` is a sample attribute from the docs, not a framework type. .NET 8 automatic antiforgery for `IFormFile` in minimal APIs belongs here or in row 13. |
| 8 versioning-openapi | Add build-time OpenAPI document generation (`Microsoft.Extensions.ApiDescription.Server`), absorbed from row 22. The multiple-documents sample appears to confuse `WithName` with document assignment (`WithGroupName`). Kiota CLI sample commands need checking. |
...
| 22 aspnet-api-documentation-strategy | Now an Architecture › Design guide; its inline links to the versioning guide are cross-category and may stay. The tooling landscape (Redoc, Stoplight, ReadMe, Scalar) fails the durable-substance test: keep the selection reasoning, drop the product catalogue. |
...
| 4 | Building APIs | aspnet-minimal-apis.md | Not started |
| 5 | Building APIs | aspnet-controller-apis.md | Not started |
| 6 | Building APIs | aspnet-validation-data-handling.md | Not started |
| 7 | Building APIs | aspnet-large-payloads.md | Not started |
| 8 | Building APIs | aspnet-versioning-openapi.md | Not started |
