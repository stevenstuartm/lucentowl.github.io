...
| Cross-service database selection, including purpose-built engines | 19 database-selection | Rows 16–18, 43: each keeps at most one clause on when it's the wrong store |
...
| 15 ebs-efs | FSx (Windows, Lustre, ONTAP, OpenZFS) is an item-3 gap; cover it at the level of what each variant is for. Check EFS performance and throughput mode guidance (Max I/O vs Elastic throughput). |
| 16 rds-aurora | Check Aurora Serverless v2 scaling to zero (late 2024) and Aurora DSQL (2025 GA) as gaps. The RDS vs Aurora framing must not repeat row 19. Its blue/green section is a mechanics section; the strategy concept belongs to `infrastructure/deployment-strategies.md`. |
| 17 dynamodb | "When to use DynamoDB vs RDS" cuts (row 19 owns). Re-verify on-demand pricing after the late-2024 price cut. Check warm throughput. Global tables material cuts to a clause (row 52 owns). |
| 18 elasticache | "When to use ElastiCache vs DAX vs CloudFront" cuts to a clause (row 19). Cache-strategy concepts cut to ElastiCache specifics. Check Valkey engine pricing and Serverless minimums. MemoryDB is absent (clause at row 19). |
| 19 database-selection | Redshift sections cut to one clause (Analytics owns). Add purpose-built engines (DocumentDB, Neptune, Keyspaces, Timestream, MemoryDB, Aurora DSQL) at clause level. The "Decision Framework Worksheet" and "Migration Considerations" may fail the Lookup Test: judge whether they teach or are a checklist. |
| 20 sqs-sns | 7.5k words with heavy internal repetition ("SQS vs SNS" appears twice, fanout three times). "Event Sourcing with SQS" is dubious, since a queue is not an event store: cut or correct. Check the SQS maximum message size (2025 lead for 1 MiB) and fair queues. |
...
| 22 step-functions | Orchestration vs choreography cuts to a clause. Check JSONata and variables (late 2024) as a gap. Re-verify the Standard vs Express limits. |
...
| 29 codedeploy | Check ECS native blue/green (2025 lead) and what it leaves CodeDeploy for on ECS. Strategy concepts cut to a clause. |
...
| 40 advanced-containers | Remove the App Mesh section; service-mesh options on AWS are now ECS Service Connect, VPC Lattice, and self-managed Istio/Linkerd on EKS. The mesh concept cuts to a clause (Architecture owns). GitOps cuts to a clause. HPA/VPA are generic Kubernetes: keep only the AWS interaction. Check EKS Pod Identity vs IRSA. |
...
| 45 data-architecture | Lake, warehouse, and lakehouse concepts cut to AWS mapping (`data/data-architecture.md` owns). Check SageMaker Lakehouse, S3 Tables/Iceberg, and zero-ETL as gaps. |
...
| 52 multi-region | Route 53 routing sections cut to a clause (row 6). "Strong consistency (Aurora Global Database)" is likely wrong, since secondaries replicate asynchronously; verify. Check DynamoDB global tables multi-Region strong consistency (2025 lead). |
...
| 15 | Storage Services | aws-ebs-efs.md | Not started |
| 16 | Database Services | aws-rds-aurora.md | Not started |
| 17 | Database Services | aws-dynamodb.md | Not started |
| 18 | Database Services | aws-elasticache.md | Not started |
| 19 | Database Services | aws-database-selection.md | Not started |
| 20 | Application Integration & Messaging | aws-sqs-sns.md | Not started |
| 21 | Application Integration & Messaging | aws-eventbridge.md | Not started |
