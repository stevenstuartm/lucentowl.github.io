...
- **EventBridge (row 21 owns).** Two custom bus kinds since September 24, 2026: the rules-and-targets bus is now "Custom Event Bus - Classic" (plus the default bus for AWS service events), and the new **Custom Event Bus** has subscribers (one target each), retention 1–365 days (1 day default), FIFO per event group, 5-minute dedup, RAM sharing, per-GB pricing, 14 Regions at launch; AWS recommends it for new applications. Events up to 1 MB since January 2026 (Classic billed per 64 KB). Classic: 5 targets per rule, retries 24 h / 185 attempts, direct cross-account targets since January 2025, CMK since May 2024, p99 ~130 ms. Scheduler replaces scheduled rules (legacy). Pipes $0.40/M after filtering.
- **Step Functions (row 22 owns).** Standard: 1 year, exactly once (except configured retries and redrive), $0.025 per 1,000 transitions (start and end count), 90-day history, redrive within 14 days. Express: 5 minutes, async at least once / sync at most once, $1/M plus $0.06/GB-hour, no `.sync`/callback/Distributed Map. JSONata and variables (November 2024) recommended for new state machines; `{% %}` in samples needs a Liquid raw wrap. 256 KiB state payload. SDK integrations reach 9,000+ actions in 200+ services. Distributed Map: 10,000 parallel children, S3 sources in the same account and Region. CMK since July 2024.
- **Kinesis and Firehose (row 23 owns).** "Amazon Data Firehose" since February 2024 (never "Kinesis Data Firehose"); "Managed Service for Apache Flink" (Kinesis Data Analytics for SQL ended January 27, 2026). KDS: records up to 10 MiB; modes On-demand Standard, On-demand Advantage (account setting, 25 MiB/s commitment, ≥60% cheaper), Provisioned; direct delivery to S3 and S3 Tables streaming tables since August 2026 (on-demand only); service-managed partition keys since September 23, 2026; 20 EFO consumers (50 on Advantage). Firehose $0.029/GB with 5 KB record rounding; MSK source delivers to S3 only.
- **CloudFormation (row 24 owns core).** `Fn::GetStackOutput` (May 2026) reads any stack's outputs across accounts and Regions as a weak reference; exports/`ImportValue` stay strong and same account-Region. Pre-deployment validation on change sets (November 2025) and on direct Create/UpdateStack (June 30, 2026, `--disable-validation`); drift-aware change sets (November 2025, `REVERT_DRIFT`); express mode (June 30, 2026, skips stabilization, rollback off by default; `cdk deploy --express`). Stack policies protect everything once set and can't be removed. Free for `AWS::` types; extensions and custom Hooks $0.0009 per handler operation. Quotas: 2,000 stacks, 500 resources, 51,200-byte direct / 1 MB S3 templates. Samples with `{{resolve:...}}` need a Liquid raw wrap.
...
- **KMS and Secrets Manager (row 31 owns).** Say "KMS key" / "customer managed key", never CMK. AWS managed keys rotate yearly (since May 2022) and can't be shared cross-account. Customer managed key rotation: default 365 days, 90–2,560 configurable, on-demand available; first two rotations +$1/month each. Symmetric crypto quota 10,000 / 20,000 / 100,000 rps by Region (us-east-1, us-west-2, eu-west-1 at 100,000). $1/key/month, $0.03 per 10,000 requests, 20,000 free. Most services re-encrypt with a destination-Region key when replicating; multi-Region keys suit client-side encryption. Secrets Manager: $0.40/secret/month, $0.05 per 10,000 calls, 64 KB, rotation as often as every four hours; managed rotation, managed external secrets, Lambda rotation. The Secrets Manager Agent is now the AWS Workload Credentials Provider. Parameter Store standard 4 KB/10,000 free; advanced 8 KB, $0.05/month, RAM sharing.
- **WAF and Shield (row 32 owns).** Console now calls web ACLs "protection packs". WAF: $5/web ACL, $1/rule, $0.60/M requests, 1,500 WCU included (max 5,000); body inspection 8 KB (ALB, AppSync) or 16 KB default up to 64 KB (CloudFront, API Gateway, Cognito, App Runner, Verified Access); no HTTP APIs, NLBs, or EC2. Rate-based rules: 1/2/5/10-minute windows, minimum 10, forwarded-IP keys use the header's first (client-set) address. Bot Control common $10 + $1/M (10M free), targeted $10 + $10/M (1M free); CAPTCHA $0.40/1,000; Challenge $0.40/M, GET text/html only; anti-DDoS rule group $20/month + $0.15/M. Shield Advanced $3,000/month, 1-year, per account billed once per payer; SRT needs Business/Enterprise support; cost protection needs a rate-based Block rule on CloudFront/ALB; Firewall Manager policies $100/policy/Region (included with Shield Advanced).
- **CloudTrail and Config (row 33 owns).** Console trails are always multi-Region, but the CLI, API, and IaC tools default to single-Region trails. CloudTrail Lake closed to new customers on May 31, 2026, and AWS directs new customers to CloudWatch. Pricing: first copy of management events free, $2/100k for further copies; data and network activity events $0.10/100k; Insights $0.35/100k management or $0.03/100k data events, per insight type. The console-created Athena table is unpartitioned and unavailable for organization trails. Config: one customer managed recorder per account per Region; continuous CIs $0.003, daily $0.012; rule and conformance pack evaluations $0.001 each for the first 100k; aggregators are free. Recorders aren't auto-enabled in new accounts, and org rules retry for only 7 hours on accounts without one.
...
| 45 data-architecture | Lake, warehouse, and lakehouse concepts cut to AWS mapping (`data/data-architecture.md` owns). Check SageMaker Lakehouse, S3 Tables/Iceberg, and zero-ETL as gaps. Row 14 gives S3 Tables, S3 Metadata (journal plus optional live inventory Iceberg tables), and S3 Vectors one line each; this row owns their analytics use. Row 23 covers the ingestion side of Kinesis delivery to S3 Tables (streaming tables, August 2026) and Firehose's Iceberg destination; state only the analytics use here. |
...
| 21 | Application Integration & Messaging | aws-eventbridge.md | Complete |
| 22 | Application Integration & Messaging | aws-step-functions.md | Complete |
| 23 | Application Integration & Messaging | aws-kinesis.md | Complete |
| 24 | Infrastructure as Code | cloudformation-fundamentals.md | Complete |
| 25 | Infrastructure as Code | cloudformation-advanced.md | Complete |
| 26 | Infrastructure as Code | aws-cdk.md | Complete |
...
| 31 | Security & Compliance | aws-kms-secrets-manager.md | Complete |
| 32 | Security & Compliance | aws-waf-shield.md | Complete |
| 33 | Security & Compliance | aws-cloudtrail-config.md | Complete |
