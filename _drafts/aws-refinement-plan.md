...
| Target row | Lead |
|---|---|
| 15 ebs-efs | FSx (Windows, Lustre, ONTAP, OpenZFS) is an item-3 gap; cover it at the level of what each variant is for. Check EFS performance and throughput mode guidance (Max I/O vs Elastic throughput). Row 14 cut its S3 vs EBS/EFS/FSx comparison entirely; the object vs block vs file choice lands here. S3 Files (April 2026, built on EFS, mounts a bucket as a file system for EC2, Lambda, EKS, ECS) belongs in that choice. |
