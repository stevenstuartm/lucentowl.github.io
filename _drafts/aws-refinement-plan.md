...
| 20 sqs-sns | 7.5k words with heavy internal repetition ("SQS vs SNS" appears twice, fanout three times). "Event Sourcing with SQS" is dubious, since a queue is not an event store: cut or correct. Check the SQS maximum message size (2025 lead for 1 MiB) and fair queues. Lambda's async payload is 1 MB, but its docs say SNS messages cap at 256 KB; re-verify SNS and SQS sizes together. |
...
| 14 | Storage Services | aws-s3-fundamentals.md | In progress |
