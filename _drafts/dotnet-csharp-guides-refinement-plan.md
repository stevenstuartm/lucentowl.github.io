...
**Current position: Phase 1, row 33.**
...
| 37 source-generators | Treats AOT as a clause per the map. The clause must match row 1: the ban is on `Reflection.Emit` and dynamic loading, not on reflection generally, and trimming is implied by AOT rather than a separate opt-in. |
