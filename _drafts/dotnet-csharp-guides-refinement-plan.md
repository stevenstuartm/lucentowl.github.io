...
**Current position: Phase 1, row 28.**
...
| 30 configuration-and-options | The named-options sample's `ApiClientFactory` takes `IOptionsSnapshot<T>`, which is scoped, so the factory can't be a singleton (row 27 fact); it also constructs a `new HttpClient` per `CreateClient` call, which row 28 owns as the socket-exhaustion antipattern. Check the registration shown and rework the sample. |
...
| 37 source-generators, 32 json-serialization | Each treats AOT as a clause per the map. The clause must match row 1: the ban is on `Reflection.Emit` and dynamic loading, not on reflection generally, and trimming is implied by AOT rather than a separate opt-in. |
